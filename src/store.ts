...
export const useAppStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      setTodoResponse: (response) => set({ todoResponse: response }),
      setActiveManualId: (id) => set({ activeManualId: id }),
      loadManual: (manual) => set({
        activeManualId: manual.id,
        extractedText: manual.extracted_text || '',
        apiResponse: manual.api_response,
        todoResponse: manual.todo_response,
        inputMode: manual.input_mode || undefined,
        currentStep: 'render' as Step,
        apiStatus: 'success' as ApiStatus,
      }),
